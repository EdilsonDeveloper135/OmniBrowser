...
> Nota posterior (2026-09-17): la [pasada de hardening tras el canvas espacial](hardening-2026-09.md) y la consolidación arquitectónica formalizan los cuatro registros de decisiones arquitectónicas ([ADR 0001 a 0004](adr/)), reemplazan los diálogos síncronos por `PromptModal` (`.native-occluder`), integran `ErrorBoundary`, unifican `sameRect` y `Favicon.tsx`, corrigen dos excepciones de dependencias (`webpack-dev-server@5.2.6` y `@electron-internal/extract-zip@1.0.5`), y amplían la verificación a **162 tests unitarios y de componentes** con soporte `@vitest/coverage-v8` y `@types/node@24.10.13`. La agrupación de avisos (F-38) cuenta con pruebas unitarias dedicadas. Este documento se conserva como registro histórico de su fecha original.
