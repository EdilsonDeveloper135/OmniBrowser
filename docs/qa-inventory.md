...
Este inventario vincula cada afirmación visible del MVP con una comprobación funcional y una evidencia visual. La suite automatizada comprende **241 pruebas unitarias y de componentes** (Vitest con `@vitest/coverage-v8`), 13 pruebas del protocolo del sidecar (`unittest`), 5 POCs deterministas en Node/Electron más la traza de compatibilidad de agentes (`npm run agent:compat`) y 36 escenarios E2E con Playwright. Las decisiones arquitectónicas subyacentes se formalizan en los registros [ADR 0001 a 0005](adr/). El almacenamiento profundo (IndexedDB, Cache Storage, service workers y caché HTTP) se cubre en los POC deterministas; la suite E2E cubre el flujo completo de la aplicación.
...
| Agentes por tarjeta | panel de chat, proveedor, cola, Pause/Resume/Stop, reinicio y cambio de perfil | el panel divide la tarjeta sin que la vista nativa lo cubra; una tarea completa con el sidecar congelado y un modelo local hace clic en la página, no roba la selección y no deja clave ni capacidad CDP en `userData`; cola limitada, interrupción y reanudación con progreso, identidad nueva al cambiar de perfil; la prueba del proveedor usa el límite de 4096 tokens de Browser Use y explica un modelo de razonamiento que lo agota o solo razona; sin sidecar, desarrollo pide `npm run agent:build`; clave solo para la sesión sin llavero ni disco (la URL y el modelo sobreviven al reinicio), sin leer el llavero al arrancar y con una denegación de macOS que deja la clave en memoria y lo explica | `runtime-regressions.spec.ts`, `agent-manager.test.ts`, `agent-store.test.ts`, `agent-worker.test.ts`, `provider-store.test.ts`, `agent-provider-modal.test.tsx`, `agent-chat-panel.test.tsx`; verificación manual con un proveedor OpenAI-compatible real en [agent-audit-2026-09.md](audits/agent-audit-2026-09.md#seguimiento-proveedor-openai-compatible-real) |
| Pasarela CDP | una tarjeta visible u oculta, dos tarjetas | solo su target; capturas con la tarjeta oculta sin cambio de visibilidad; `file:`, cookies, pestañas y target vecino denegados; URL y título actualizados tras navegar; frames perdidos durante una navegación reintentados | `npm run agent:compat`, `scoped-cdp-gateway.test.ts` |
...
Estado a 2026-09-17 ([hardening-2026-09](audits/hardening-2026-09.md)). Se ejecutan con la app empaquetada, un `userData` nuevo y la fixture local; una casilla solo se marca con la arquitectura, el sistema y el resultado anotados.
